...
def t_check(cname):
    try:
        sql01="select count(*) from db_cmdb.ha_mysql_table_list\
                 where cluster_name='%s' and role='slave'"%(cname)
        sql02="select count(*) from db_cmdb.ha_mysql_table_list\
                where no_master=0 and cluster_name='%s'"%(cname)
        cslave=get_mdb(sql01)
        cno=get_mdb(sql02)
        if cslave[0][0] >= 2:
            if cno[0][0] >= 1:
                ti=tb_info(cname)
                ...
                print "%s At least one has permission to ascend"%(cname)
                sys.exit(1)
        else:
            print "%s slave Must be greater than one"%(cname)
            sys.exit(1)
...
def vip_check(cname):
    try:
        switch=find_vip(cname)
        return switch[1]

    except Exception as err:
        print "vip_check: {}".format(err)
        sys.exit(1)
def get_cli_options():
